	...
	<?php echo '<div '.$wdata->get_render_attribute_string( 'elth-inner' ).'>';?>
    	...
    	foreach (  $list_sliders as $key => $item ) {            
            ...
            echo '<div '.$wdata->get_render_attribute_string( 'info_attr' ).'>
                    ';
            if($item['title']) echo '<h3 class="item-title"><a href="'.$link.'"'.$target.$nofollow.'>'.$item['title'].'</a></h3>';
            if($item['description']) echo '<p class="item-des">'.$item['description'].'</p>';
            if($item['content']) echo '<p class="item-content">'.$item['content'].'</p>';
            if($item['button_text']) echo '<a class="elth-bt-default elth-bt-medium '.$item['button_style'].'" href="'.$item['link']['url'].'"'.$target.$nofollow.'>'.$item['button_text'].'</a>';
            echo        '</div>
                    </div>';
            if($item['style_info']){

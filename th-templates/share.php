...
if((isset($check_share[$post_type]) && $check_share[$post_type] == '1') || $check_share_page == '1'):
	$list_default = array(
		array(
			'title'  => esc_html__('Total',"coffeehouse"),
		    'social' => 'total',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Facebook',"coffeehouse"),
		    'social' => 'facebook',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Twitter',"coffeehouse"),
		    'social' => 'twitter',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Google',"coffeehouse"),
		    'social' => 'google',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Pinterest',"coffeehouse"),
		    'social' => 'pinterest',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Linkedin',"coffeehouse"),
		    'social' => 'linkedin',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Tumblr',"coffeehouse"),
		    'social' => 'tumblr',
		    'number' => '1',
			),
		array(
			'title'  => esc_html__('Email',"coffeehouse"),
		    'social' => 'envelope',
		    'number' => '1',
			),
		);
		...
		foreach ($list as $value) {
			switch ($value['social']) {
				...
				case 'envelope':
					...
					echo 	'<li><a target="_blank" data-social="'.esc_attr($value['social']).'" title="'.esc_attr($value['title']).'" href="mailto:?subject='.esc_attr__("I wanted you to see this site&amp;body=Check out this site","coffeehouse").' '.get_the_permalink().'">
								<span class="share-icon '.esc_attr($value['social']).'-social"><i class="la la-'.esc_attr($value['social']).'" aria-hidden="true"></i>'.$number_html.'</span>
							</a></li>';
					break;

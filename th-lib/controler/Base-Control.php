    ...
    class Th_BaseController{
                ...
                foreach($add_sidebars as $sidebar){
                    ...
                    if(!empty($sidebar['title'])){
                        ...
                        $custom_add_sidebar = array(
                                'name' => $sidebar['title'],
                                'id' => $id,
                                'description' => esc_html__( 'SideBar created by add sidebar in theme options.', 'coffeehouse'),
        ...
        static function th_product_cat_metabox_add($tag) { 
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content Before','coffeehouse'); ?></label>
                <div class="wrap-metabox">
                    <select name="before_append" id="before_append">
                        <option value=""><?php esc_html_e("Choose page","coffeehouse")?></option>
                        <?php
            ...
            <div class="form-field">
                <label><?php esc_html_e('Append Content After','coffeehouse'); ?></label>
                <div class="wrap-metabox">
        ...
        if($th_woo_widgets != 'true' && isset($active_widgets['woocommerce-sidebar']) && empty($active_widgets['woocommerce-sidebar'])){
            ...
            $widgets = array(
                'woocommerce_product_categories' => array(
                    'title' => esc_html__('Product categories','coffeehouse'),
                    'orderby' => 'name',
                    'dropdown' => 0,
                    'count' => 0,
                    'hierarchical' => 1,
                ...
                'woocommerce_price_filter' => array(
                    'title' => esc_html__('Filter by price','coffeehouse'),
                    ),
                'woocommerce_products' => array(
                    'title' => esc_html__('Products','coffeehouse'),
                    'number' =>  5,
                    'show' => '',
                    'orderby' => 'date',
                    'order' => 'desc',

...
if(class_exists("woocommerce")){
    ...
    if(!function_exists('th_change_price_html')){
        function th_change_price_html($price, $product){
            ...
            $type_class = '';
            if(is_object($product) && method_exists($product,'get_type')) $type_class = $product->get_type();
    ...
    if(!function_exists('th_woocommerce_pagination')){
        function th_woocommerce_pagination(){
            ...
            if($shop_style == 'load-more' && $max_page > 1){
                ...
                if(is_array($curent_tax_query)) $args['tax_query'] = $curent_tax_query;
                if(is_array($curent_meta_query)) $args['meta_query'] = $curent_meta_query;
                ...
                echo    '<div class="btn-loadmore">
                            <a href="#" class="product-loadmore loadmore elth-bt-default elth-bt-medium" 
                                ...
                                '.esc_html__("Load more","coffeehouse").'
                            </a>
                        </div>';
            }
    ...
    if(!function_exists('th_product_quickview')){
        function th_product_quickview($icon = '<i class="la la-search"></i>',$class = '',$echo = true) {
            $html = '<a title="'.esc_attr__("Quick View","coffeehouse").'" data-product-id="'.get_the_id().'" href="'.esc_url(get_the_permalink()).'" class="product-quick-view quickview-link '.esc_attr($class).'">'.$icon.'</a>';
    ...
    if(!function_exists('th_product_label')){
        function th_product_label($echo = true) {
            ...
            if($uppsell < 0) $html .=   '<span class="new">'.esc_html__("new","coffeehouse").'</span>';
    ...
    if ( !function_exists( 'th_catalog_ordering' ) ) {
        function th_catalog_ordering($query,$set_orderby = '',$list_item = false,$add_class = '') {        
            ...
            $catalog_orderby_options = apply_filters( 'woocommerce_catalog_orderby', array(
                'menu_order' => esc_html__( 'Default sorting', 'coffeehouse' ),
                'popularity' => esc_html__( 'Sort by popularity', 'coffeehouse' ),
                'rating'     => esc_html__( 'Sort by average rating', 'coffeehouse' ),
                'date'       => esc_html__( 'Sort by newness', 'coffeehouse' ),
                'price'      => esc_html__( 'Sort by price: low to high', 'coffeehouse' ),
                'price-desc' => esc_html__( 'Sort by price: high to low', 'coffeehouse' )
            ) );
            ...
            if ( wc_get_loop_prop( 'is_search' ) ) {
                $catalog_orderby_options = array_merge( array( 'relevance' => esc_html__( 'Relevance', 'coffeehouse' ) ), $catalog_orderby_options );
        ...
        function th_load_product_filter() {
            ...
            if(isset($pagination) && !empty($pagination)){?>
                <div class="products row list-product-wrap js-content-main">
                    ...
                    else echo '<div class="filter-noresult-wrap"><div class="filter-noresult title18 text-center">'.esc_html__("No result found with current filter value.","coffeehouse").'</div></div>';
                    ?>
                </div>
                <?php
...
if(!function_exists('th_live_search')){
    function th_live_search() {
        ...
        if( $query->have_posts() && !empty($key) && !empty($trim_key)){
            ...
            echo '<p class="text-center">'.esc_html__("No any results with this keyword.","coffeehouse").'</p>';
        }
        wp_reset_postdata();
    }
}
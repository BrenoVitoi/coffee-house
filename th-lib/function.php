...
if(!function_exists('th_list_post_type')){
    function th_list_post_type($post_type = 'page',$add_empty = false,$convert = false){
        ...
        $page_list = array();
        if($add_empty){
            if(!$convert) $page_list[''] = esc_html__('-- Choose One --','coffeehouse');
            else{
                $page_list[] = array(
...
if ( !function_exists( 'th_get_google_link' ) ) {
    function th_get_google_link() {
        $font_url = '';
        $fonts  = array(
                    'Economica:400,500,700',
                    'Quicksand:300,400,500,600,700',
                );
        if ( 'off' !== _x( 'on', 'Google font: on or off', 'coffeehouse' ) ) {
                ...
                switch ($preload_style) {
                    ...
                    case 'custom-image':
                        ...
                        <div id="loading-center-absolute-image">
                            <img src="<?php echo esc_url($preload_img)?>" alt="<?php esc_attr_e("preload-image","coffeehouse");?>"/>
                        </div>
                        <?php
                        break;
                            ...
                            switch ($value) {
                                ...
                                case 'tags':
                                    ...
                                    if($tags):?>
                                        <li class="meta-item"><i class="la la-tags" aria-hidden="true"></i>
                                            ...
                                            <?php if($tags) echo apply_filters('th_output_content',$tags); else esc_html_e("No Tag",'coffeehouse');?>
                                        </li>
                                        <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                    <?php endif;
                                    break;
                                ...
                                case 'comments':
                                    ...
                                    <li class="meta-item"><i aria-hidden="true" class="la la-comment"></i>
                                        ...
                                        <?php 
                                            if(get_comments_number() != 1) esc_html_e('Comments', 'coffeehouse') ;
                                            else esc_html_e('Comment', 'coffeehouse') ;
                                        ?>
                                        </a>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
                                ...
                                case 'views':
                                    ...
                                    <li class="meta-item"><i class="la la-eye"></i>
                                        ...
                                        if(th_get_post_view() != 1) echo esc_html__("Views",'coffeehouse');
                                        else echo esc_html__("View",'coffeehouse');
                                        ?>
                                        </span>
                                    </li>
                                    <?php if($key < (count($data)-1) && $split) echo '<li class="split">'.$split.'</li>';?>
...
if(!function_exists('th_get_post_list_style')){
    function th_get_post_list_style(){
        $list = apply_filters('th_post_list_item_style',array(
            ...
            'style2' => esc_html__('Post list 2','coffeehouse'),
            'style3' => esc_html__('Post list 3','coffeehouse'),
            'style4' => esc_html__('Post list 4','coffeehouse'),
            'style5' => esc_html__('Post list 5','coffeehouse'),
            ));        
        return $list;
    }
}
if(!function_exists('th_get_post_style')){
    function th_get_post_style($style = 'element'){
        $list = apply_filters('th_post_item_style',array(
            '' => esc_html__('Default','coffeehouse'),
            'style2' => esc_html__('Post grid 2','coffeehouse'),
            ));
...
if(!function_exists('th_get_product_list_style')){
    function th_get_product_list_style(){
        $list = apply_filters('th_product_list_item_style',array(
            ''          => esc_html__('Default','coffeehouse'),
            'style2'    => esc_html__('Product list 2','coffeehouse'),
            ));
        return $list;
    }
}
if(!function_exists('th_get_product_style')){
    function th_get_product_style($style = 'element'){
        $list = apply_filters('th_product_item_style',array(
            ''          => esc_html__('Default','coffeehouse'),
            'style2'    => esc_html__('Product grid 2','coffeehouse'),
            'style3'    => esc_html__('Product grid 3','coffeehouse'),
            'style4'    => esc_html__('Product grid 4','coffeehouse'),
            ));
        return $list;
    }
}
...
if(!function_exists('th_fill_css_typography')){
    function th_fill_css_typography($data,$important = ''){
        $style = '';
        $subsets = '';
        if(!empty($data['subsets'])) $subsets = ' , '.$data['subsets'];
        if(!empty($data['color'])) $style .= 'color:'.$data['color'].$important.';';
        if(!empty($data['font-family'])) $style .= 'font-family:'.$data['font-family'].$subsets.$important.';';
...
if(!function_exists('th_compare_url')){
    function th_compare_url($icon='',$id = false,$text = '',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Compare","coffeehouse");
...
if(!function_exists('th_wishlist_url')){
    function th_wishlist_url($icon='',$text='',$class='silver'){
        $html = '';
        if(empty($text)) $text = esc_html__("Wishlist","coffeehouse");
...
if(!function_exists('th_breadcrumb')){
    function th_breadcrumb($step = '') {
        global $post;
        if(is_home() && !is_front_page()) echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','coffeehouse').'</a>'.$step.'<span>'.esc_html__('Blog','coffeehouse').'</span>';
        else echo '<a href="'.esc_url(home_url('/')).'">'.esc_html__('Home','coffeehouse').'</a>';
        if (is_single()){
        ...
        elseif(is_search()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("Search Results for: ","coffeehouse").get_search_query().'</span>';
        elseif(is_404()) echo apply_filters('s7upf_output_content',$step)."<span>".esc_html__("404 ","coffeehouse")."</span>";
    }
}
if(!function_exists('th_fix_import_category')){
    function th_fix_import_category($taxonomy){
...
if(!function_exists('th_register_user')){
    function th_register_user(){
        ...
        $output = array();
        $output['status'] = 'error';
        $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username or email has been exist. Please check again. ","sportify").'</div></div>';
        if(!empty($user_data)){
            if(!username_exists($user_data['user_login']) && !email_exists($user_data['user_email'])){                
                $user_id = wp_insert_user( $user_data );
                if(class_exists('WC_Emails')){
                    $wc_email = new WC_Emails();
                    $wc_email->customer_new_account($user_id);
                }
                $output['status'] = 'success';
                $output['message'] = '<div class="message login_info ms-info ms-default">'.esc_html__("Account registration has been successful","sportify").'</div>';
            }            
            if(username_exists($username)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your username has been exist.","sportify").'</div></div>';
            }           
            if(username_exists($email)){
                $output['status'] = 'error';
                $output['message'] = '<div class="message-wrap"><div class="message login_error ms-error ms-default">'.esc_html__("Your email has been exist.","sportify").'</div></div>';
            }
        }
        echo ''.$output['message'];
        die();

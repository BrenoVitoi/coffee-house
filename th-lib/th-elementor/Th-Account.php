...
class Th_Account extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Account manager', 'coffeehouse' );
	}
	...
	public function login_form($redirect_to = '') {
        ...
        <div class="elth-login-form popup-form active">
            <div class="form-header">
                <h2><?php esc_html_e( 'Log In','coffeehouse' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Become a part of our community!','coffeehouse' ); ?></div>
                <div class="message ms-done ms-default"><?php esc_html_e( 'Registration complete. Please check your email.','coffeehouse' ); ?></div>
            </div>
            <form name="loginform" id="loginform" action="<?php echo esc_url( site_url( 'wp-login.php', 'login_post' ) ); ?>" method="post">
                <?php do_action( 'woocommerce_login_form_start' ); ?>
                <div class="form-field">
                    <input placeholder="<?php esc_html_e( 'Username or Email Address','coffeehouse' ); ?>" type="text" name="log" id="user_login" class="input" size="20" autocomplete="off"/>
                </div>
                <div class="form-field">
                	<input placeholder="<?php esc_html_e( 'Password','coffeehouse' ); ?>" type="password" name="pwd" id="user_pass" class="input" value="" size="20" autocomplete="off"/>
                </div>
                ...
                <div class="forgetmenot">
                    ...
                    <label class="rememberme" for="remembermep"><?php esc_html_e( 'Remember Me','coffeehouse' ); ?></label>
                </div>
                <div class="submit">
                    <input type="submit" name="wp-submit" class="elth-bt-default elth-bt-full" value="<?php esc_attr_e('Log In','coffeehouse'); ?>" />
            ...
            <div class="nav-form">
                <?php if ( get_option( 'users_can_register' ) ) :
                    echo '<a href="#registerform" class="popup-redirect register-link">'.esc_html__("Register","coffeehouse").'</a>';
                endif;
                echo '<a href="#lostpasswordform" class="popup-redirect lostpass-link">'.esc_html__("Lost your password?","coffeehouse").'</a>';
                ?>
            </div>
        </div>
        <?php
	...
	public function register_form($redirect_to = '') {
        ...
        <div class="elth-register-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Create an account','coffeehouse' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Welcome! Register for an account','coffeehouse' ); ?></div>
                <div class="message login_error error_1 ms-error ms-default"><?php esc_html_e( 'The user name or email address is not correct.','coffeehouse' ); ?></div>
                <!-- fix popup register -->
                <div class="message login_error error_2 ms-error ms-default"><?php esc_html_e( 'The password is not correct.','coffeehouse' ); ?></div>
                
            </div>
            <form name="registerform" id="registerform" action="<?php echo esc_url( site_url( 'wp-login.php?action=register', 'login_post' ) ); ?>" method="post" novalidate="novalidate">
                <?php do_action( 'woocommerce_register_form_start' ); ?>
                <div class="form-field">
                    <input placeholder="<?php esc_html_e('Username','coffeehouse') ?>" type="text" name="user_login" id="user_loginr" class="input" value="" size="20" autocomplete="off"/>
                </div>
                <div class="form-field">
                    <input placeholder="<?php esc_html_e('Email','coffeehouse') ?>" type="email" name="user_email" id="user_email" class="input" value="" size="25" autocomplete="off"/>
                </div>
                <?php if ( 'no' === get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div class="form-field">
                        <input placeholder="<?php esc_html_e( 'Password', 'coffeehouse' ); ?>" type="password" name="user_pass" id="reg_password" autocomplete="new-password" />
                    </div>
                <?php }?>
                <div class="extra-field">
                    <?php 
                ...
                <?php if ( 'no' != get_option( 'woocommerce_registration_generate_password' ) ){ ?>
                    <div id="reg_passmail">
                        <?php esc_html_e( 'Registration confirmation will be emailed to you.','coffeehouse' ); ?>
                    </div>
                <?php }?>
                <div class="submit"><input type="submit" name="wp-submit" class="elth-bt-default elth-bt-full" value="<?php esc_attr_e('Register',"coffeehouse"); ?>" /></div>
                <?php do_action( 'woocommerce_register_form_end' ); ?>
            </form>

            <div class="nav-form">
                <a href="#loginform" class="popup-redirect login-link"><?php esc_html_e( 'Log in','coffeehouse' ); ?></a>
                <a href="#lostpasswordform" class="popup-redirect lostpass-link"><?php esc_html_e( 'Lost your password?','coffeehouse' ); ?></a>
            </div>
        </div>
        <?php
	}
	public function lostpass_form($redirect_to = '') {
        ...
        <div class="elth-lostpass-form popup-form">
            <div class="form-header">
                <h2><?php esc_html_e( 'Reset password','coffeehouse' ); ?></h2>
                <div class="desc"><?php esc_html_e( 'Recover your password','coffeehouse' ); ?></div>
                <div class="message ms-default ms-done"><?php esc_html_e( 'Password reset email has been sent.','coffeehouse' ); ?></div>
                <div class="message login_error ms-error ms-default"><?php esc_html_e( 'The email could not be sent.
Possible reason: your host may have disabled the mail function.','coffeehouse' ); ?></div>
            </div>
            <form name="lostpasswordform" id="lostpasswordform" action="<?php echo esc_url( network_site_url( 'wp-login.php?action=lostpassword', 'login_post' ) ); ?>" method="post">
                <div class="form-field">
                    <input placeholder="<?php esc_html_e( 'Username or Email Address','coffeehouse' ); ?>" type="text" name="user_login" id="user_loginlp" class="input" value="" size="20" autocomplete="off"/>
                </div>
                <div class="extra-field">
                    <?php do_action( 'lostpassword_form' ); ?>
                    <input type="hidden" name="redirect_to1" value="<?php echo esc_attr( $redirect_to ); ?>" />
                </div>
                <div class="submit"><input type="submit" name="wp-submit" class="elth-bt-default elth-bt-full" value="<?php esc_attr_e('Get New Password','coffeehouse'); ?>" /></div>
                <div class="desc note"><?php esc_html_e( 'A password will be e-mailed to you.','coffeehouse' ); ?></div>
            </form>
	...
	protected function _register_controls() {
		...
		$this->add_control(
			...
			[
				...
				'options'   => [
					'elth-account-icon'		=> esc_html__( 'Icon', 'coffeehouse' ),
				],
			]
		);
		...
		$this->add_control(
			'icon_logged',
			[
				'label' => esc_html__( 'Icon logged', 'coffeehouse' ),
				'type' => Controls_Manager::ICONS,
				'default' => [
					'value' => 'la la-user',
					'library' => 'solid',
		...
		$this->start_controls_section(
			'section_links',
			[
				'label' => esc_html__( 'Links', 'coffeehouse' ),
				'tab' => Controls_Manager::TAB_CONTENT,
			]
		);
		...
		$repeater_text->add_control(
			'roles',
			[
				'label' => esc_html__( 'Show with roles', 'coffeehouse' ),
				'description' => esc_html__( 'Choose roles to show. Default is show with all roles', 'coffeehouse' ),
		...
		$this->add_control(
			'list_links',
			[
				'label' => esc_html__( 'Add links', 'coffeehouse' ),
		...
		$this->add_responsive_control(
			'wrap_links_left',
			[
				'label' => esc_html__( 'Wrap Left', 'coffeehouse' ),
				'type' => Controls_Manager::CHOOSE,
				'default'	=> '',
				'options' => [
					'0' => [
						'title' => esc_html__( '0', 'coffeehouse' ),
						'icon' => 'eicon-text-align-left',
					],
					'inherit' => [
						'title' => esc_html__( 'Inherit', 'coffeehouse' ),
		...
		$this->add_responsive_control(
			'wrap_links_right',
			[
				'label' => esc_html__( 'Wrap Right', 'coffeehouse' ),
				'type' => Controls_Manager::CHOOSE,
				'default'	=> '',
				'options' => [
					'0' => [

        ...
        static function _do_import()
        {
            if(isset($_REQUEST['th_do_import']) and $_REQUEST['th_do_import'] || isset($_REQUEST['media']))
            {
                ...
                if(!current_user_can('manage_options')){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('You do not have permission to do this','coffeehouse')));die;
                }
                self::load_lib();

                //Check Importer Plugins was installed
                if ( !class_exists( 'WP_Importer' ) or ! class_exists( 'WP_Import' ) ){
                    echo json_encode(array('status'=>0,'message'=>esc_html__('Importer Class Was Not Installed','coffeehouse')));die;
                }
                global $th_config;
                    ...
                    if(!empty($data_option)){
                        ...
                        if(!function_exists('th_get_option_name'))
                        {
                            echo json_encode( array(
                                    'status'   =>0,
                                    'messenger'=>"<span class='red'>".esc_html__("Plugin: 7up core must be installed first. Stop working!","coffeehouse")."</span>",
                                    'next_url' => ''
                                )
                            );
                            die;
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Importing the demo theme options...","coffeehouse")."<span>".esc_html__("DONE!","coffeehouse")."</span><br>",
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Theme option contain NULL content. Stop working!","coffeehouse")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==2){
                    ...
                    if(!empty($data_object)){
                        ...
                        echo json_encode( array(
                                'status'   =>1,
                                'messenger'=>esc_html__("Importing the demo widgets...","coffeehouse")." <span>".esc_html__("DONE!","coffeehouse")."</span>.<br>",
                                'next_url' => admin_url(self::$_import_page."&th_do_import=1&media={$import_media}&package={$package}&step=" . ($step + 1)),
                            )
                        );
                    }
                        ...
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("Widget option contain NULL content. Stop working!","coffeehouse")."</span>",
                                'next_url' => ''
                            )
                        );
                    }
                ...
                if($step==3){
                    ...
                    if(!$file_name){
                        echo json_encode( array(
                                'status'   =>0,
                                'messenger'=>"<span class='red'>".esc_html__("File Not Found. Stop working!","coffeehouse")."</span>",
                                'next_url' => ''
                            )
                        );
                        die;
                ...
                if($step==4) {
                    ...
                    echo json_encode(array(
                            'status' => 1,
                            'messenger' => esc_html__("Importing menu settings ...","coffeehouse")." <span>".esc_html__("DONE!","coffeehouse")."</span><br>",
                            'next_url' => $nexturl,
                        )
                    );
                }
                ...
                if($step == '5'){
                    ...
                    if($stt_file<count($ds_file) && class_exists('RevSlider')){   
                        ...
                        if(!$set_woocommerce_page) {
                            $next_url = '';
                            $messenger = '<span>'.esc_html__("All Done! Have Fun","coffeehouse").'</span>';
                        }
                        else {
                        ...
                        echo json_encode( array(
                                'status'   =>"ok",
                                'messenger'=>esc_html__("Setting reading options...","coffeehouse")." <span>".esc_html__("DONE!","coffeehouse")."</span><br/>".$messenger,
                                'next_url' => $next_url,
                            )
                        );
                    }
                ...
                if($step=='6')
                {
                    ...
                    if(class_exists('Elementor\Core\Admin\Admin')){
                        ...
                        $args = array("post_type" => "elementor_library", "s" => 'Default Kit');
                        $query = get_posts( $args );
                        if(isset($query[0])){
                            $settings = json_decode($th_config['elementor_settings'],true);
                            update_post_meta($query[0]->ID,'_elementor_page_settings',$settings);
                        }
                        wp_reset_postdata();
                    }                 
                    echo json_encode( array(
                            'status'   =>"ok",
                            'messenger'=>esc_html__("Setting WooCommerce options...","coffeehouse")." <span>".esc_html__("DONE!","coffeehouse")."</span><br/><span>".esc_html__("All Done! Have Fun","coffeehouse")."</span>",
                            'next_url' => '',
                        )
                    );
                }
